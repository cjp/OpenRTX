...
static const struct spiConfig spiC6000Cfg =
{
    .clk       = { C6K_CLK  },
    .mosi      = { C6K_MOSI },
    .miso      = { C6K_MISO },
    .clkPeriod = SCK_PERIOD_FROM_FREQ(2000000),
    .flags     = SPI_FLAG_CPHA
};

static pthread_mutex_t c6000_spi_mutex;

SPI_BITBANG_DEVICE_DEFINE(det_spi,   NULL, spiDetCfg)
SPI_BITBANG_DEVICE_DEFINE(pll_spi,   NULL, spiPllCfg)
SPI_BITBANG_DEVICE_DEFINE(c6000_spi, &c6000_spi_mutex, spiC6000Cfg)

static const struct ak2365a detector =
{
    .spi = &det_spi,
    .cs  = { DET_CS  },
    .res = { DET_RST }
...
static const struct sky73210 pll =
{
    .spi    = &pll_spi,
    .cs     = { PLL_CS },
    .refClk = 16800000
...
static const struct Cx000Interface c6000If
{
    .uSpi  = &c6000_spi,
    .uCs   = { C6K_CS }
};
...
static CS760Calib calData;                      // Calibration data
...
HR_C6000& C6000 = HR_C6000::instance(&c6000If);  // HR_C5000 driver
...
void radio_disableRtx()
{
    ...
    gpioDev_clear(TX_PWR_EN);   // Disable TX PA
    gpioDev_clear(RX_PWR_EN);   // Disable RX LNA
    gpioDev_clear(AF_MUTE);     // Mute analog path towards the audio amplifier
