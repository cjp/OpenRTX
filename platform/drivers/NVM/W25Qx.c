 ...
 */
static int waitUntilReady(const struct w25qCfg *cfg, uint32_t timeout)
{
    ...
    while(timeout > 0)
    {
        ...
        spi_transfer(cfg->spi, cmd, 2, ret, 2);
        gpioPin_set(&cfg->cs);
...
{
    const struct w25qCfg *cfg = (const struct w25qCfg *) dev->priv;
...
{
    const struct w25qSecRegDevice *pDev = (const struct w25qSecRegDevice *) dev;
    const struct w25qCfg *cfg = (const struct w25qCfg *) dev->priv;

    uint32_t addrBase  = offset & 0x3000;
    uint32_t addrRange = offset & 0xCFFF;

    if(addrBase != pDev->baseAddr)
        return -EINVAL;
